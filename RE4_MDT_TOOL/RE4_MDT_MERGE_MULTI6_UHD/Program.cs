...
namespace RE4_MDT_MERGE_MULTI6_UHD
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("RE4_MDT_MERGE_MULTI6_UHD");
        ...
        static void Action(string file)
        {
            ...
            var (multiLang, outputFullName) = RE4_MDT_MERGE_MULTI.MakeMergedMulti.MakeMergedMulti_All(fileInfo.FullName, false, false); //UHD multi6

            var outputFileInfo = new FileInfo(outputFullName);
            var outStream = outputFileInfo.OpenWrite();
            RE4_MDT_PARSE.MakeMDT.Make_UHD_BIG(multiLang, outStream); //UHD multi6
            outStream.Close();
        }
    }
}
