...
namespace RE4_MDT_MERGE_MULTI_NS
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("RE4_MDT_MERGE_MULTI_NS");
        ...
        static void Action(string file)
        {
            ...
            var (multiLang, outputFullName) = RE4_MDT_MERGE_MULTI.MakeMergedMulti.MakeMergedMulti_All(fileInfo.FullName, true, true); //NS

            var outputFileInfo = new FileInfo(outputFullName);
            var outStream = outputFileInfo.OpenWrite();
            RE4_MDT_PARSE.MakeMDT.Make_UHD_NS(multiLang, outStream, true); //NS
            outStream.Close();
        }
    }
}
