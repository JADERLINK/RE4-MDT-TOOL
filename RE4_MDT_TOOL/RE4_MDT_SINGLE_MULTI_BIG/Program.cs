...
namespace RE4_MDT_SINGLE_MULTI_BIG
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("RE4_MDT_SINGLE_MULTI_BIG");
        ...
        static void Action(string file)
        {
            ...
            var res = RE4_MDT_PARSE.ParseMDT.ParseMono(stream, 0, stream.Length, false, SimpleEndianBinaryIO.Endianness.BigEndian); // BIG
            stream.Close();

            var outputFileInfo = new FileInfo(outputFile);
            var outStream = outputFileInfo.OpenWrite();
            RE4_MDT_SINGLE_MULTI.MakeSingleMulti.MakeSingleMulti_UHD_BIG(res, outStream, SimpleEndianBinaryIO.Endianness.BigEndian); // BIG
            outStream.Close();
        }
    }
}
